...
public class QuizzesController: ControllerBase
{
    ...
    public async Task<ActionResult<int>> SubmitQuiz([FromBody] QuizSubmission submission)
    {
        ...
        var quiz = await _context.Quizzes.Include(q => q.Questions!).ThenInclude(q => q.Answers!).FirstOrDefaultAsync(q => q.Id == submission.QuizId);
    ...
    private int CalculateScore(Quiz quiz, QuizSubmission submission)
    {
        ...
        foreach (var question in quiz.Questions!)
        {
            ...
            {
                if (question.Answers!.Any(a => a.Id == submissionAnswer.AnswerIds!.FirstOrDefault() && a.IsCorrect))
                {
                    score += 100;
                }
            }
            ...
            {
                var correctAnswers = question.Answers.FirstOrDefault(a => a.IsCorrect)?.Text.Trim().ToLower();
